...
export function GlobalHeader({ activeTab, onTabChange }: GlobalHeaderProps) {
  return (
    <header className="h-14 bg-bg border-b border-border grid grid-cols-3 items-center px-4">
      {/* 로고 */}
      <span className="text-lg font-bold tracking-tight text-text-primary select-none">
        NEWSSTAND
      </span>

      {/* MY뉴스 / 전체언론사 토글 — 뷰포트 정중앙 */}
      <div className="flex justify-center">
        <div className="flex border border-border rounded text-sm font-bold overflow-hidden">
      ...
      <div className="flex justify-end gap-3 text-text-secondary">
        <button type="button" aria-label="새로고침" className="hover:text-text-primary transition-colors duration-150">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M3 12a9 9 0 1 0 9-9 9 9 0 0 0-6.36 2.64L3 8" /><path d="M3 3v5h5" />
          </svg>
        </button>
        <button type="button" aria-label="설정" className="hover:text-text-primary transition-colors duration-150">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
