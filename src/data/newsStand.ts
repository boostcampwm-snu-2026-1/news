...
export const PUBLISHERS: readonly Publisher[] = publisherSeeds.map((publisher) => ({
  ...publisher,
  articles: createArticles(publisher.id, publisher.name),
}))

export const INITIAL_SUBSCRIBED_PUBLISHER_IDS = [] as const satisfies readonly Publisher['id'][]

export const TICKER_ITEMS: readonly TickerItem[] = PUBLISHERS.slice(0, 8).map(
  (publisher, index) => ({
    id: `ticker-${index + 1}`,
